...
class BankbranchController extends Controller
{
    ...
    public function add_banks_branch(Request $request){
        ...
        $bankData = $request->all();
         // Check if the bank and branch already exist
        $existingBankBranch = Bankbranch::where('bank_id', $bankData['bank_id'])
        ->where('branch_id', $bankData['branch_id'])
        ->first();

        if ($existingBankBranch) {
            return response()->json(['error' => 'Bank and branch already exist.'], 422);
        }else{
    ...
    public function view_banks_branch ($id){
        ...
        $bank = Bankbranch::with('bank', 'branch')
        ->where('id', $id)
        ->first();
    ...
    public function getBankwiseBranch($bankId){

        $bank = Bank::find($bankId); // Assuming you have the bank ID
        $branches = $bank->branches()->get();
